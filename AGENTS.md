...
- Keep temporary test outputs out of the project after verification. Formal explorer output belongs in `outputs/dfs_result`; animation output belongs in `outputs/animation`; `outputs/` and `references/` are ignored.
...
- For current hat explorer runs, use reflection, output to `outputs/dfs_result`, and restrict glued edge lengths with `--allowed-length-pairs "1:1,1:2,2:2,sqrt3:sqrt3"`. After explorer runs, update `outputs/dfs_result/tiles_trace.png`.
